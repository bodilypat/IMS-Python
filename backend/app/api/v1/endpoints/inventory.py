...
def update.inventory(inventory_id: int, inventory_in: schemas.inventory.InventoryUpdate, db: Session = Depends(get_db)):
	inventory = crud.inventory.get(db, id=inventory_id)
	if not inventory:
		raise HTTPException(status_cod=status.HTTP_404_NOT_FOUND, detail="Inventory not found")
	return crud.inventory.update(db=db, db_obj=inventory, obj=inventory_in)
