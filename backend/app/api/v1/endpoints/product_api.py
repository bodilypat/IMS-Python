...
def get_product_by_id(
	...
	if not product:
		raise HTTPException(status_code=404, detail="Product not found"0
	return product 
...
def create_product(
	...
	"""
	existing = product_service.get_product_by_id(db, product_in.sku)
	if existing:
...
def update_product(
		product_id: int,
		product_update: product_schema.ProductResponse,
		db: Session = Depends(get_db)
	) -> product_schema.ProductResponse:
	"""
	...
	return product_service.update_product(db, db_product, product_update)
	
@router.delete("/{product_id}", response_model=status.HTTP_201_NO_CONTENT)
def delete_product(
		product_id: int,
		db: Session = Depends(get_db)
	) -> None:
	...
	if not db_product:
		raise HTTPException(status_code=404, detail="Product not found.")
	product_service.soft_delete_product(db, db_product)
	return None 
